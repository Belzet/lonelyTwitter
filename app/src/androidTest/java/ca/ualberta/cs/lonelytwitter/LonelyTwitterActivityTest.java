...
public class LonelyTwitterActivityTest extends ActivityInstrumentationTestCase2 {
    ...
    public void testEditATweet() {
        ...
        activity.runOnUiThread(new Runnable() {
            public void run() {
                saveButton.performClick(); //acts like we just clicked the button
            }
        });
        getInstrumentation().waitForIdleSync(); // make sure our UI thread finishes

        final ListView oldTweetsList = activity.getOldTweetsList();
        Tweet tweet = (Tweet) oldTweetsList.getItemAtPosition(0); //doesn't need to run on UI thread because we aren't changing anything
